...
class LambdaLayer(nn.Module):
    def __init__(
        ...
        *,
        dim_k,          # key dimension
        n = None,       # size of the receptive window - max(height, width) - input pixels
        r = None,       # the receptive field for relative positional encoding (23 x 23)
    ...
    def forward(self, x):
        print("=====> size of x: ")
        print(x.size())     #   torch.Size([850, 6, 256]) number 6 is batch_size
        print("=====> shape of x: ")
        print(x.shape)      #   torch.Size([850, 6, 256]) number 6 is batch_size
